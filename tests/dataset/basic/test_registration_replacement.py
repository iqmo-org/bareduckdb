...
class TestRegistrationReplacement:
    ...
    def test_simple_replacement(self):
        ...
        conn.register("my_table", table1)

        result = conn.sql("SELECT * FROM my_table ORDER BY id").fetchall()
        assert len(result) == 3
        assert result[0] == (1, 'a')
        assert result[1] == (2, 'b')
        assert result[2] == (3, 'c')
    ...
    def test_replacement_with_different_schema(self):
        conn = Connection()
        ...
        conn.register("data", table1)

        result = conn.sql("SELECT * FROM data ORDER BY id").fetchall()
        assert len(result) == 2
    ...
    def test_replacement_preserves_other_tables(self):
        ...
        result_a = conn.sql("SELECT count(*) FROM table_a").fetchone()
        result_b = conn.sql("SELECT count(*) FROM table_b").fetchone()
        assert result_a[0] == 2
        assert result_b[0] == 2

        new_table_a = pa.table({'id': [100, 200, 300], 'name': ['NEW1', 'NEW2', 'NEW3']})
        conn.register("table_a", new_table_a)

        result_a = conn.sql("SELECT count(*) FROM table_a").fetchone()
        assert result_a[0] == 3, f"Expected table_a to have 3 rows after replacement, got {result_a[0]}"

        result_b = conn.sql("SELECT * FROM table_b ORDER BY id").fetchall()
    ...
    def test_replacement_with_uuid_data(self):
        conn = Connection()

        table1 = pa.table({
            'uuid': ['00000000-0000-0000-0000-000000000000', 'ffffffff-ffff-ffff-ffff-ffffffffffff']
        })
        conn.register("arrow_table", table1)

        result = conn.sql("SELECT * FROM arrow_table ORDER BY uuid").fetchall()
        assert len(result) == 2
        ...
        conn.register("arrow_table", table2)

        result = conn.sql("SELECT * FROM arrow_table").fetchall()
    ...
    def test_replacement_with_filter(self):
        conn = Connection()
        ...
        result = conn.sql("SELECT count(*) FROM data WHERE category = 'A'").fetchone()
        assert result[0] == 2
        ...
        result = conn.sql("SELECT id FROM data WHERE category = 'A' ORDER BY id").fetchall()
