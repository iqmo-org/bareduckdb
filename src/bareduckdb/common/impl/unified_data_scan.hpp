...
struct HolderFactory {
    ...
    HolderFactory(
        ...
        , release_capsule_callback(release_callback)
        , column_names(col_names)
        , num_rows(rows)
        , supports_views(supports_views_)
        , schema_capsule_pyobj(nullptr)
    {
    ...
    void ParseStats(size_t stats_count, const ColumnStatsInput* stats) {
        if (stats_count == 0 || !stats) return;

        precomputed_stats.resize(column_names.size());

        for (size_t i = 0; i < stats_count; i++) {
            int col_idx = stats[i].col_index;
            if (col_idx < 0 || col_idx >= static_cast<int>(precomputed_stats.size())) continue;
            ...
            if (stats[i].min_str) ps.min_str = stats[i].min_str;
            if (stats[i].max_str) ps.max_str = stats[i].max_str;
        }
    }
